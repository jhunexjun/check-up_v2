...
{
    public partial class frmGRPO : Form
    {
        DataTable table;
        DataTable dt;
        ...
        private void frmGRPO_Load(object sender, EventArgs e)
        {
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            {
                sql = "SELECT docId,vendorCode,vendorName,warehouse,DATE_FORMAT(postingDate, '%m/%d/%Y') postingDate,totalPrcntDscnt,totalAmtDscnt,ROUND(netTotal,2) netTotal,ROUND(grossTotal,2) grossTotal,remarks1,remarks2 FROM grpo WHERE ";

                if (txtGRPONo.Text.Trim() != "" && !txtGRPONo.Text.Contains("*"))
                {
                    sql += "docId='" + txtGRPONo.Text.Trim().Replace("'", "''") + "'";
                    db = new database();
                    dt = new DataTable();
                    dt = db.select(sql, vars.MySqlConnection);
                    if (dt.Rows.Count < 1)
                    ...
                    txtGRPONo.Text = dt.Rows[0]["docId"].ToString();
                    ...
                    sql = "SELECT * FROM grpo_item WHERE docId='" + txtGRPONo.Text.Trim() + "' ORDER BY indx";
                    db = new database();
                    dt = db.select(sql, vars.MySqlConnection);
                ...
                else
                {
                    ...
                    frmDialogForm.selectedValue = sql;
                    frmDialogForm.ShowDialog();
                    txtGRPONo.Text = frmDialogForm.selectedValue;
                }
            }
            ...
            {
                db = new database();
                sql = "UPDATE grpo SET remarks2='" + txtRemarks2.Text.Trim().Replace("'", "''") + "',updateDate=DATE_FORMAT(NOW(), '%Y-%m-%d %H:%i:%s'),updatedBy='" + vars.username + "' WHERE docId='" + txtGRPONo.Text.Trim() + "'";
            ...
            else if (btnFind.Text == "&Save")
            {
                if (checkValues())
                {
                    ...
                    sql += "SET @newId=(SELECT CAST(lastNo+1 AS char(11)) FROM documents WHERE documentCode='GRPO');";
                    ...
                    sql += "INSERT INTO grpo(docId,vendorCode,vendorName,warehouse,postingDate,totalPrcntDscnt,totalAmtDscnt,netTotal,grossTotal";
                    ...
                    for (i = 0; i < rowCount; i++)
                    {
                        if (!dgvItems.Rows[i].IsNewRow && dgvItems.Rows[i].Cells[0].Value.ToString() != "")
                        {
                            ...
                            sql += "INSERT INTO grpo_item(docId,indx,itemCode,description,warehouse,vatable,realBsNetPrchsPrc,realBsGrossPrchsPrc,realNetPrchsPrc,realGrossPrchsPrc,qty,baseUoM,qtyPrPrchsUoM,prcntDscnt,amtDscnt,netPrchsPrc,grossPrchsPrc,rowNetTotal,rowGrossTotal)";
                    ...
                    if (db.executeNonQuery(sql, vars.MySqlConnection) > 0)
                    {
                        ...
                        db = new database(); dt = new DataTable();
                        dt = db.select("SELECT docId FROM grpo ORDER BY id DESC LIMIT 1", vars.MySqlConnection);
                        txtGRPONo.Text = dt.Rows[0][0].ToString();
        ...
        private void registerControls()
        {
            ...
            table.Rows.Add("txtGRPONo", "docId", "");
        ...
        private void linkVendorName_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            {
                sql += " like '" + txtVendorName.Text.Replace("*", "%").Trim() + "' ORDER BY BPName";
                frmDialogForm = new frmDialog();
                frmDialogForm.selectedValue = sql;
                frmDialogForm.ShowDialog();
                txtVendorCode.Text = frmDialogForm.selectedValue;
        ...
        {
            crystal_reports.GRPO objReport;
            this.Cursor = Cursors.WaitCursor;

            sql = "SELECT docId,vendorCode,vendorName, A.warehouse, DATE_FORMAT(postingDate, '%m/%d/%Y') postingDate, totalPrcntDscnt, totalAmtDscnt, netTotal,grossTotal,remarks1,remarks2,B.* ,C.prchsUoM  FROM grpo A JOIN grpo_item B USING(docId) JOIN itemmasterdata C USING(itemCode) WHERE docId='" + txtGRPONo.Text.Trim() + "' ORDER BY indx";
            db = new database();
            dt = new DataTable();
            dt = db.select(sql, vars.MySqlConnection);

            objReport = new crystal_reports.GRPO();
            ...
            crystalReportViewerForm.Text = "GRPO Report";
