...
namespace Check_up.forms
{
    public partial class frmPO : Form
    {
        ...
        public frmPO()
        {
            ...
            dgvItems.CellLeave += new DataGridViewCellEventHandler(dgvItems_CellLeave);
        }
        ...
        void dgvItems_CellLeave(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (dgvItems.Columns[e.ColumnIndex].Name == "itemCode" && !dgvItems.Rows[e.RowIndex].IsNewRow)
                    dgvItems.Rows[e.RowIndex].Cells["baseUoM"].Value = "N";

                if (dgvItems.Columns[e.ColumnIndex].Name == "description" && !dgvItems.Rows[e.RowIndex].IsNewRow)
                    dgvItems.Rows[e.RowIndex].Cells["baseUoM"].Value = "N";
        ...
        private Hashtable getInfo(string itemCode)
            {
                ...
                sql += " WHERE itemCode='" + itemCode + "' AND pricelistCode=0 AND deactivated='N' AND vendor='" + txtVendorCode.Text + "'";
        ...
        private void populateDataGridRow(Hashtable ht, int rowIndex)
        {
            ...
            decimal d;
            d = Decimal.Parse(ht["qtyPrPrchsUoM"].ToString());
            dgvItems.Rows[rowIndex].Cells["qtyPrPrchsUoM"].Value = d.ToString(vars.format);

            d = Decimal.Parse(ht["realBsNetPrchsPrc"].ToString());
            dgvItems.Rows[rowIndex].Cells["realBsNetPrchsPrc"].Value = d.ToString(vars.format);
        }
        ...
        private bool itemCodeIsFound(object p)
        {
            ...
            if (btnFind.Text == "&Save")
            {
                if (dgvItems.Rows[e.RowIndex].Cells[e.ColumnIndex].Value != null)
                {
                    ...
                    if (dbColumnName == "itemCode" || dbColumnName == "description")
                    {
                        ...
                        sql += " FROM itemmasterdata A JOIN pricelist B USING(itemCode)";
                        sql += " WHERE pricelistCode=0 AND deactivated='N' AND vendor='" + txtVendorCode.Text + "'";
        ...
        private bool checkValues()
        {
            ...
            {
                MessageBox.Show(this, "No item(s) to save.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                dgvItems.Focus();
                return false;
            }
            DateTime dateTime;
            ...
            for (i = 0; i < rowCount; i++)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        MessageBox.Show(this, "Item code " + itemCode + " is deactivated. Cannot proceed.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return false;
                    }
                }
                else
            ...
            {
                MessageBox.Show(this, "Please choose warehouse.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); ;
                cboWarehouse.Focus();
                return false;
            }
        ...
        private void setToAfterSaveMode()
        {
            txtPostingDate.ReadOnly = false;
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            if (btnFind.Text == "&Find")
            {
                sql = "SELECT docId,vendorCode,vendorName,warehouse,DATE_FORMAT(postingDate, '%m/%d/%Y') postingDate,totalPrcntDscnt,totalAmtDscnt,ROUND(netTotal,2) netTotal, ROUND(grossTotal,2) grossTotal,remarks1,remarks2 FROM purchaseorder WHERE ";

                if (txtPONo.Text.Trim() != "" && !txtPONo.Text.Contains("*"))
                {
                    sql += "docId='" + txtPONo.Text.Trim().Replace("'", "''") + "'";
                    db = new database();
                    dt = new DataTable();
                    dt = db.select(sql, vars.MySqlConnection);
                    if (dt.Rows.Count < 1)
                    ...
                    txtPONo.Text = dt.Rows[0]["docId"].ToString();
                    ...
                    txtGrossTotal.Text = d.ToString(vars.grossFormat);


                    sql = "SELECT * FROM purchaseorder_item WHERE docId='" + txtPONo.Text.Trim() + "' ORDER BY indx";
                    db = new database();
                    dt = db.select(sql, vars.MySqlConnection);
                    ...
                    for (i = 0; i < rowCount; i++ )
                    {
                        ...
                        d = Decimal.Parse( dt.Rows[i]["realBsNetPrchsPrc"].ToString());
                        dgvItems.Rows[i].Cells["realBsNetPrchsPrc"].Value = d.ToString(vars.format);

                        d = Decimal.Parse(dt.Rows[i]["Qty"].ToString());            
                        dgvItems.Rows[i].Cells["Qty"].Value = d.ToString(vars.format);

                        d = Decimal.Parse(dt.Rows[i]["qtyPrPrchsUoM"].ToString());
                        dgvItems.Rows[i].Cells["qtyPrPrchsUoM"].Value = d.ToString(vars.format);
                        ...
                        d = Decimal.Parse(dt.Rows[i]["prcntDscnt"].ToString());
                        dgvItems.Rows[i].Cells["prcntDscnt"].Value = d.ToString(vars.format);
                        ...
                        d = Decimal.Parse(dt.Rows[i]["rowNetTotal"].ToString());
                        dgvItems.Rows[i].Cells["rowNetTotal"].Value = d.ToString(vars.format);
                ...
                else
                {
                    ...
                    frmDialogForm.selectedValue = sql;
                    frmDialogForm.ShowDialog();
                    txtPONo.Text = frmDialogForm.selectedValue;
                }
            }
            ...
            else if (btnFind.Text == "&Save")
            {
                if (checkValues())
                {
                    ...
                    sql += "INSERT INTO purchaseorder(docId,vendorCode,vendorName,warehouse,postingDate,totalPrcntDscnt,totalAmtDscnt,netTotal,grossTotal";
                    if (txtRemarks1.Text.Trim() != "")
                        sql += ",remarks1";
                    if (txtRemarks2.Text.Trim() != "")
                        sql += ",remarks2";
                    sql += ",createDate,createdBy)";
                    sql += " VALUES(@docId,@vendorCode,'" + txtVendorName.Text.Trim().Replace("'", "''") + "','" + cboWarehouse.Text + "',@postingDate,@totalPrcntDscnt,@totalAmtDscnt,@netTotal,@grossTotal";
                    if (txtRemarks1.Text.Trim() != "")
                    ...
                    for (i = 0; i < rowCount; i++)
                    {
                        if (!dgvItems.Rows[i].IsNewRow && dgvItems.Rows[i].Cells[0].Value.ToString() != "")
                        {
                            ...
                            decimal varQty, varPrcntDscnt, varAmtDscnt, varNetPrchsPrc, varGrossPrchsPrc, varRowNetTotal, varRowGrossTotal, varQtyPrPrchsUoM, varRealNetPrchsPrc, varRealGrossPrchsPrc, varRealBsNetPrchsPrc, varRealBsGrossPrchsPrc;
                            ...
                            sql += "INSERT INTO purchaseorder_item(docId,indx,itemCode,description,vatable,realBsNetPrchsPrc,realBsGrossPrchsPrc,realNetPrchsPrc,realGrossPrchsPrc,qty,baseUoM,qtyPrPrchsUoM,prcntDscnt,amtDscnt,netPrchsPrc,grossPrchsPrc,rowNetTotal,rowGrossTotal)";
                    ...
                    sql += "UPDATE businesspartner SET trans='Y' WHERE code=@vendorCode;";
                    sql += "UPDATE documents SET lastNo=CAST(@newId AS UNSIGNED) WHERE documentCode='PO';";
                    sql += "COMMIT;";
                    ...
                    if (db.executeNonQuery(sql, vars.MySqlConnection) > 0)
                    {
                        ...
                        db = new database(); dt = new DataTable();
                        dt = db.select("SELECT docId FROM purchaseorder ORDER BY id DESC LIMIT 1", vars.MySqlConnection);
        ...
        private void linkVendorCode_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            if (txtVendorCode.Text.Trim() != "" || txtVendorCode.Text.Contains("*"))
            {
                ...
                sql = "SELECT code,BPName FROM businesspartner WHERE BPType=0 AND deactivated='N' AND code='" + txtVendorCode.Text.ToString() + "'";
                db = new database(); dt = new DataTable();
                dt = db.select(sql, vars.MySqlConnection);
        ...
        {
            crystal_reports.PO objReport;
            this.Cursor = Cursors.WaitCursor;

            sql = "SELECT docId,vendorCode,vendorName,DATE_FORMAT(postingDate, '%m/%d/%Y') postingDate,totalPrcntDscnt,totalAmtDscnt,netTotal,grossTotal,remarks1,remarks2,B.* FROM purchaseorder A JOIN purchaseorder_item B USING(docId) WHERE docId='" + txtPONo.Text.Trim() + "'";

            db = new database();
            dt = new DataTable();
            dt = db.select(sql, vars.MySqlConnection);

            objReport = new crystal_reports.PO();
            ...
            crystalReportViewerForm.Text = "Purchase Order Report";
