...
{
    public partial class frmSearchGRPO : Form
    {
        DataTable dt;
        database db;
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT code,BPName FROM businesspartner WHERE BPType = 0 AND deactivated = 'N' ORDER BY BPName");
        ...
        {
            // dtPickerDate.Value.ToString("MM/dd/yyyy");
            if (txtDateFrm.Text.Trim() == "" || txtDateTo.Text.Trim() == "")
            {
                if (txtDateFrm.Text.Trim() == "")
