...
{
    public partial class frmGRPOReport : Form
    {
        DataTable dt;
        database db;
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            ...
            if (chkGRPO.Checked == false && chkVendor.Checked == false && chkDate.Checked == false)
            {
            ...
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT itemCode,description,realBsGrossPrchsPrc,qty,rowGrossTotal FROM grpo JOIN grpo_item USING(docId) WHERE");
            if (chkGRPO.Checked == true)
                sb.Append(" docId >= '" + txtGRPONoFrm.Text.Trim() + "' AND docId <= '" + txtGRPONoTo.Text.Trim() + "'");
            if ((chkGRPO.Checked == true && chkVendor.Checked == true) || (chkGRPO.Checked == true && chkDate.Checked == true))
                sb.Append(" AND");
        ...
        private void linkLabelGRPO_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT docId,vendorCode,vendorName,postingDate,warehouse,grossTotal FROM grpo ORDER BY docId DESC");
            if (txtGRPONoFrm.Text.Trim() == "" || txtGRPONoTo.Text.Trim() == "")
            {
                frmDialogForm = new frmDialog();
                frmDialogForm.selectedValue = sb.ToString();
                frmDialogForm.ShowDialog();
                if (txtGRPONoFrm.Text.Trim() == "")
                    txtGRPONoFrm.Text = frmDialogForm.selectedValue;
                else
                    txtGRPONoTo.Text = frmDialogForm.selectedValue;

                if (txtGRPONoFrm.Text.Trim() != "" && txtGRPONoTo.Text.Trim() != "")
                    linkLabelGRPO.Enabled = false;
            }
        }
        ...
        private void chkGRPO_CheckedChanged(object sender, EventArgs e)
        {
            if (chkGRPO.Checked == true)
            {
                txtGRPONoFrm.Enabled = true;
                txtGRPONoTo.Enabled = true;
                linkLabelGRPO.Enabled = true;
            }
            else
            {
                txtGRPONoFrm.Enabled = false;
                txtGRPONoTo.Enabled = false;
                linkLabelGRPO.Enabled = false;
            }
