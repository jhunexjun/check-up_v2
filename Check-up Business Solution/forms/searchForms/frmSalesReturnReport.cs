...
namespace Check_up.forms
{
    public partial class frmSalesReturnReport : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (chkSalesReturnInvoiceNo.Checked == false && chkCustomer.Checked == false && chkDate.Checked == false)
            {
            ...
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT itemCode,description,realBsGrossSalePrc,qty,rowGrossTotal FROM salesreturn JOIN salesreturn_item USING(docId) WHERE");
            if (chkSalesReturnInvoiceNo.Checked == true)
                sb.Append(" docId >= '" + txtSalesReturnInvoiceNoFrm.Text.Trim() + "' AND docId <= '" + txtSalesReturnInvoiceNoTo.Text.Trim() + "'");
            if ((chkSalesReturnInvoiceNo.Checked == true && chkCustomer.Checked == true) || (chkSalesReturnInvoiceNo.Checked == true && chkDate.Checked == true))
                sb.Append(" AND");
        ...
        private void frmSearchSalesReturn_Load(object sender, EventArgs e)
        {
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT docId,customerCode,customerCode,postingDate,warehouse,grossTotal FROM salesreturn ORDER BY docId DESC");
            if (txtSalesReturnInvoiceNoFrm.Text.Trim() == "" || txtSalesReturnInvoiceNoTo.Text.Trim() == "")
            {
                frmDialogForm = new frmDialog();
                frmDialogForm.selectedValue = sb.ToString();
                frmDialogForm.ShowDialog();
                if (txtSalesReturnInvoiceNoFrm.Text.Trim() == "")
                    txtSalesReturnInvoiceNoFrm.Text = frmDialogForm.selectedValue;
                else
                    txtSalesReturnInvoiceNoTo.Text = frmDialogForm.selectedValue;

                if (txtSalesReturnInvoiceNoFrm.Text.Trim() != "" && txtSalesReturnInvoiceNoTo.Text.Trim() != "")
                    linkLabelSalesReturnInvoiceNo.Enabled = false;
            }
        }
        ...
        private void chkSalesInvoiceNo_CheckedChanged(object sender, EventArgs e)
        {
            if (chkSalesReturnInvoiceNo.Checked == true)
            {
                txtSalesReturnInvoiceNoFrm.Enabled = true;
                txtSalesReturnInvoiceNoTo.Enabled = true;
                linkLabelSalesReturnInvoiceNo.Enabled = true;
            }
            else
            {
                txtSalesReturnInvoiceNoFrm.Enabled = false;
                txtSalesReturnInvoiceNoTo.Enabled = false;
                linkLabelSalesReturnInvoiceNo.Enabled = false;
            }
        ...
        {
            if (txtSalesReturnInvoiceNoTo.Text.Trim() == "")
                linkLabelSalesReturnInvoiceNo.Enabled = true;
        }
